...
```{r model}
   ...
   penguins_clust_all <- 
      tibble (k = 1:15) %>%
      mutate (
         cluster = map(k, ~kmeans(penguins[4:6], .x)),
...
#Identify optimal number of clusters
```{r}
   penguins_clusterings %>%
      filter (residual_type != "total") %>%
      ggplot (aes(x = k, y= residual_sum, fill = residual_type)) +
         geom_col () +
