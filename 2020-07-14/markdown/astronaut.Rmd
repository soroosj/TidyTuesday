...
```{r transform}
   ...
   astronaut <- astronaut_raw %>%
      ...
      mutate (
         ...
         name_life = glue ("{year_of_birth} - {year_of_mission}\n({life})"),
      ...
      select (name_life, nationality_code, number_global_nation, mission_title, occupation, missions_count, missions_hours, photo) %>%
      head(10)
```
...
```{r visualize_med}
   ...
   ggplot (astronaut) +
      geom_text(
         aes (x = 0, y = 1.25, label = missions_hours),
