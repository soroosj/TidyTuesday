...
```{r transform, message = F, results = F}
   ...
   visits <- visits_raw %>%
      ...
      mutate (
         ...
         unit_code = str_remove (unit_code, " National Park")
         ) %>%
      arrange (-year, -visitors) %>%
      group_by (year) %>%
         mutate (
            z_visitors = round(scale (visitors),2),
            rank_visitors = rank(desc(visitors), ties.method = "first")
               ) %>%
         do(head(., n = 10)) %>%
      ungroup (year)
...
```{r}
   ...
   ggplot(data = visits, mapping = aes(year, y = rank_visitors, group = unit_code, color = unit_code)) +
     ...
     geom_text_repel(data = visits5, aes(label = unit_code), hjust = "inward", size = 3, nudge_x = 8) +
     geom_text_repel(data = visits6, aes(label = unit_code), hjust = "inward", size = 3, nudge_x = -5) +
     labs (
       title = "U.S. National Park Visitors (1995 - 2015)",
      ...
      theme(
         ...
         plot.subtitle = element_text(hjust = 0, vjust = 0, size = 9, face = "bold", margin = margin (0,0,25,0)),
