...
```{r}
   ...
   y0 = min(na.omit(combine$year)) # Get base year.
   
   combine = combine %>% 
      mutate(y = year - y0, # Adjust years to run from 0.
            logt = log(cpu_actual)) # Add log of transistor column for a quick join.

   mod = lm(log(cpu_actual) ~ y, # Run model for linear fit.
         combine)
