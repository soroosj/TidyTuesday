   ...
   plot_grade <- 
      ggplot (coffee_grades, aes (x = grade_type, y = reorder(country_of_origin, grade_med))) +
         ...
         theme (
            ...
            panel.grid.minor = element_blank(),
            axis.title = element_text (size = 8, hjust = 0.5, color = "white"),
            axis.text.x = element_text (size = 8, hjust = 0.5, color = "white", angle = 90),
            axis.text.y = element_blank (),
...
```{r transform, warning = TRUE, results = FALSE, message = FALSE}
   ...
   plot_distrib + plot_count + plot_grade + plot_layout(widths = c(1, .25, .25)) +
   plot_annotation (
      ...
      theme = theme (
         ...
         plot.caption = element_text (hjust = 1, size = 8, margin = margin (20,0,0,0), color = "white")
         )
      )
   ggsave("coffee.png")
``` 