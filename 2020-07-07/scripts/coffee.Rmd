   ...
   plot_median <-
      ggplot (coffee_summ, aes (x = 1, y = reorder(country_of_origin, rating_med))) +
         ...
         theme (
            ...
            axis.title = element_text (size = 8, hjust = 0.5, vjust = 1, color = "white", angle = 80),
            axis.text.x = element_text (size = 8, hjust = 0, vjust = 0, color = "white"),
...
```{r transform, warning = TRUE, results = FALSE, message = FALSE}
   ...
   plot_annotation (
      title = "World Coffee Competition",
      caption = "Visualization: Joel Soroos @soroosj  |  Data: Coffee Quality Database via Kaggle via R4DS Tidy Tuesday",
      theme = theme (
         plot.background = element_rect(fill= "#6F4E37", color = "#6F4E37"),
         plot.title = element_text(hjust = 0.5, vjust = 0, size = 14, face = "bold", margin = margin (0,0,0,0), color = "white"),
         plot.subtitle = element_text (hjust = 0.5, vjust = 0, size = 10, margin = margin (0,0,25,0), color = "white"),
         plot.caption = element_text (hjust = 0, size = 7, margin = margin (10,0,0,0), color = "white")
         )
      )
   ggsave("coffee.png")
``` 