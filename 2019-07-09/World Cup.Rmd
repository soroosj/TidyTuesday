----
   title: "Women's World Cup Historical Reslts"
   author: "Joel Soroos"
   date: "July 20, 2019"
   output: pdf_document
---
...
```{r transform2, message = F}
   ...
   outcomes <- outcomes_raw %>%
     filter (
        team %in% teams_2019,                             #only include teams in 2019 World Cup.  
        round != "Third Place Playoff"
         ) %>%
     mutate(
        year = as.factor (year),
...
```{r chart, warning = F, message = F}
   ...
   ggplot (data = outcomes, aes(x = game_team_id, y = final, fill = win_status)) +
      geom_col () +
